...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        if ik_config.get("target_builder") == "mos_smplx":
            if src_human != "smplx" or tgt_robot != "mos":
                raise ValueError("MOS anatomical calibration requires SMPL-X to MOS")
            # A taller human requires less scaling to fit the same robot.
            ratio = 1.0 / ratio
        ...
        if ik_config.get("target_builder") == "mos_smplx":
            from .utils.mos import MosTargets
            self.target_builder = MosTargets(self.model, self.human_scale_table["pelvis"])
    ...
    def update_targets(self, human_data, offset_to_ground=False):
        ...
        if self.target_builder is not None:
            human_data = self.target_builder(human_data)
        else:
    ...
    def retarget(self, human_data, offset_to_ground=False):
        ...
        first_mos_frame = self.target_builder is not None and not self._mos_initialized
        if first_mos_frame:
            q = self.configuration.data.qpos.copy()
            q[:3], q[3:7] = self.scaled_human_data["pelvis"]
            self.configuration.update(q)
            self._mos_initialized = True
        max_iter = 100 if first_mos_frame else self.max_iter
    ...
    def error1(self):
        if self.target_builder is not None:
            # MOS has zero-cost position targets. They must not stop convergence
            # of the weighted orientation/foot objective actually solved by IK.
            return np.linalg.norm(np.concatenate([
                task.cost * task.compute_error(self.configuration) for task in self.tasks1
            ]))
    ...
    def offset_human_data_to_ground(self, human_data):
        ...
        for body_name in human_data.keys():
            ...
            is_mos_ankle = self.target_builder is not None and body_name.endswith("_ankle")
            if "Foot" not in body_name and "foot" not in body_name and not is_mos_ankle:
                continue
            pos, quat = human_data[body_name]
            if pos[2] < lowest_pos:
                lowest_pos = pos[2]
