...
def draw_skeleton(human_motion_data, viewer, joint_radius=0.025, bone_radius=0.012):
    """Draw a compact SMPL-X-style stick skeleton in the viewer scene."""
    edges = [
    ...
    points = {
        name: value[0]
        for name, value in human_motion_data.items()
        if name in main_joints
    }
    for parent, child in edges:
        ...
        mj.mjv_initGeom(
            ...
            size=np.asarray([bone_radius, bone_radius, bone_radius], dtype=np.float64),
            pos=np.asarray((points[parent] + points[child]) * 0.5, dtype=np.float64),
            mat=np.eye(3, dtype=np.float64).flatten(),
            rgba=np.asarray([0.05, 0.85, 1.0, 0.9], dtype=np.float32),
        )
        mj.mjv_connector(geom, type=mj.mjtGeom.mjGEOM_CAPSULE,
                         width=bone_radius, from_=points[parent], to=points[child])
    ...
    for pos in points.values():
        ...
        mj.mjv_initGeom(
            ...
            size=np.asarray([joint_radius, 0.0, 0.0], dtype=np.float64),
            pos=np.asarray(pos, dtype=np.float64),
            mat=np.eye(3, dtype=np.float64).flatten(),
            rgba=np.asarray([1.0, 0.65, 0.05, 1.0], dtype=np.float32),
        )
        viewer.user_scn.ngeom += 1
...
class RobotMotionViewer:
    ...
    def step(self, 
        ...
        if human_motion_data is not None:
            ...
            if human_skeleton:
                skeleton_data = {
                    name: (pos + human_pos_offset, rot)
                    for name, (pos, rot) in human_motion_data.items()
                }
                draw_skeleton(
                    skeleton_data,
                    self.viewer,
                    joint_radius=human_point_scale * 0.28,
                    bone_radius=human_point_scale * 0.12,
                )
            else:
